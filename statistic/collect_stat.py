...
class CollectStatistics:
        ...
        if self.is_single_run:
            ...
            with open(self.results_file_name, 'a') as f:
                f.write(str(case) + ',' + str(total_time_recomputed) + ',' + str(loss_value) + ','
                        ...
                        + str(self.beta_adapts[-1]) + ',' + str(
                            self.delta_adapts[-1]) + ',' + str(self.rho_adapts[-1])
                        + ',' + str(tau) + ',' + str(it_each_local) + ',' + str(it_each_global) + '\n')
                f.close()

    def collect_stat_end_local_round_standard_fl(self, case, it_each_local, it_each_global, model, train_image,
                                                 train_label, test_image, test_label, w_global, total_time_recomputed):
        ...
        if self.is_single_run:
            ...
            with open(self.results_file_name, 'a') as f:
                f.write(str(case) + ',' + str(total_time_recomputed) + ',' + str(loss_value) + ','
                        + str(prediction_accuracy) + ',' + ',' + str(it_each_local) + ',' + str(it_each_global) + '\n')
                f.close()
    ...
    def collect_stat_end_global_round(self, sim, case, tau_setup, total_time, model, train_image, train_label,
        ...
        if not self.is_single_run:
            ...
            avg_tau = np.sum(np.dot(taus_array, taus_array)) / \
                np.sum(taus_array)
            ...
            with open(self.results_file_name, 'a') as f:
                f.write(type_str + ',' + str(sim) + ',' + str(case) + ',' + str(tau_setup) + ','
                        + str(loss_final) + ',' + str(accuracy_final) +
                        ',' + str(avg_tau) + ',' + str(stddev_tau) + ','
                        + str(avg_each_local) + ',' +
                        str(stddev_each_local) + ','
                        + str(avg_each_global) + ',' +
                        str(stddev_each_global) + ','
                        + str(avg_beta_adapt) + ',' +
                        str(stddev_beta_adapt) + ','
                        + str(avg_delta_adapt) + ',' +
                        str(stddev_delta_adapt) + ','
                        + str(avg_rho_adapt) + ',' +
                        str(stddev_rho_adapt) + ','
